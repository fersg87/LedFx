...
class GradientEffect(Effect):
    ...
    # Transient colour override, see set_gradient_override(). Never persisted
    # to config: it shadows _config["gradient"] until cleared.
    _gradient_override: Optional[str] = None
    ...
    def set_gradient_override(self, gradient_str: str):
        """Shadow the configured gradient with *gradient_str*.

        The effect keeps running: only the colour source of the gradient curve
        changes, so roll, modulation and beat response are untouched. The
        effect's own config is not modified, so clear_gradient_override()
        restores the original colours exactly.

        Args:
            gradient_str: a colour or gradient string, as accepted by the
                effect's ``gradient`` config key.
        """
        with self.lock:
    ...
    def clear_gradient_override(self):
        """Restore the gradient from the effect's own config."""
        with self.lock:
    ...
    def get_gradient_color_vectorized1d(self, points):
        ...
        # Transpose and reshape to get the correct shape (length, color_channels)
